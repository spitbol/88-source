...
/
/   V1.02 4-Sep-91	Add code for OS/2 DLLs.
*/
...
#if MSDOS | SYSTEM5 | IRIX | AIX | SOLARIS | LINUX
#include <fcntl.h>
#endif
...
word fd;
{
#if MSDOS & (HIGHC | BCC16 | BCC32 | VCC | INTEL)
	close(fd);
#endif					/* HIGHC | BCC16 | INTEL */
}
...
char *file;
{
	...
	if (lnscb->len == 0)			/* If no library name, use */
    {					/* function name with ".slf" extension */
	    savecp = fnscb->str + fnscb->len;	/* Make it a C string for now. */
	    savechar  = make_c_str(savecp);
	    appendext(fnscb->str,EFNEXT,file,1);
	}
	else
	{
	    savecp = lnscb->str + lnscb->len;	/* Make it a C string for now. */
	    savechar  = make_c_str(savecp);
	    mystrcpy(file,lnscb->str);
	}
	mystrcpy(file2,file);
	fd = spit_open( file, O_RDONLY, IO_PRIVATE | IO_DENY_WRITE,
	 IO_OPEN_IF_EXISTS ); /* Open file */
	if (fd < 0) {				/* If couldn't open */
	    initpath(SPITFILEPATH);	/* try alternate paths */
	...
	unmake_c_str(savecp, savechar);			/* Restore saved char */
	return fd;
#endif					/* HIGHC | BCC16 | INTEL */
	 ...
	 *     If not found, then
	 *        Use unmodified function name (OS/2 will append .dll
	 *        extension and search along LIBPATH).  Note:  This may
