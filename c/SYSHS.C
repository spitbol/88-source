...
/                    *** AWAITING INSTALLATION OF TYPE_RCL ***
#endif
/
...
#if BCC16
#define host386 host88
#endif

#if !(HIGHC | INTEL | OS2 | BCC32 | VCC | BCC16)
extern uword lowxs;
#endif
	    ...
	    switch( (int)val ) {
			...
			if ( getint(icp,&val) ) {
			    ...
			    SET_XR( pTICBLK );
				 switch ( (int)val ) {
			         ...
			         return EXIT_8;
				 	case 5:							/* stack in use */
#if HIGHC | INTEL | OS2 | BCC32 | VCC | BCC16 | LINUX
			...
			*/
		    case 1: {
				char *cmd, *path;
#if APPLE
				/* HOST(1, "command", "path", msg) */
				IATYPE msg = 0L;
				if (!getint( WB(struct icblk *), &msg ) || msg < 0 || msg > 1)
					return EXIT_1;							/* Bad 4th arg */
#endif					/* APPLE */
...
#if APPLE
				/* HOST(1,"cmd","path",msg) */
				pTICBLK->val = -1;					/* Prepare for failure */
				if (cmd) {
					flush_all(GET_MIN_VALUE(R_FCB,struct chfcb *));	/* In case not MultiFinder*/
					if (doLaunch(1, cmd, (int)msg, path))
						pTICBLK->val = 0;				/* Sub-launch OK */
					}
#else					/* APPLE */
