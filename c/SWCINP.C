...
/					  alert user.
/
/	01.15	18-Oct-90 Remove AMIGA conditionals, consolidate with APPLE version.
/
...
/
/		This problem only occurs on systems using dup().  Apple
/		does not use fd 0 for source file input, and are immune.
/
...
/
/		For systems that do not support dup(), like APPLE, we leave
...

{
	...
	if ( curfile++ < filecnt ) {
		/*
		/  First call, with source file fd in originp.
		*/
		setrdfd( originp );
		return originp;
	}
