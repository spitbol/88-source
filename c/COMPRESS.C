...
#if APPLE
#define sbrk compressAlloc
extern char *sbrk Params(( long size ));
#endif					/* APPLE */
...
uword size;
{
	...
	if (bits == BITS)			/* turn on expansion */
	{
		if (EMEMORY <= size)
...
word fd;
{
	...
	return_value = (unsigned int)(bit_buffer >> (32-BITS));
	bit_buffer <<= BITS;
	bit_count -= BITS;
	return return_value;
}
...
uword size;
{
	...
	if (bits == BITS)			/* turn on compression */
	{
		if (CMEMORY <= size)
			extra = 0;	/* no extra memory needed */
		else
